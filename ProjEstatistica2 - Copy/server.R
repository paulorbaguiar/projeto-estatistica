...
server <- function(input, output) {
    ...
    Info_DataTable_COMP <- eventReactive(input$go_comp,{
        ...
        Mediana_1 <- median(vetor_1)
        DP_1 <- sd(vetor_1)
        Moda_1 <- getmode(vetor_1)
        
        Media_2 <- mean(vetor_2)
        Mediana_2 <- median(vetor_2)
        DP_2 <- sd(vetor_2)
        Moda_2 <- getmode(vetor_2)
        
        Dados <- input$column
        
        df_tb <-  data.frame(Dados, Media_1, Mediana_1, DP_1, Moda_1, Media_2, Mediana_2, DP_2, Moda_2)
        df_tb <- as.data.frame(t(df_tb))
